...
class Chat:
    def __init__(self, params, router_obj: Optional[Any]):
        ...
        if self.params.get("acompletion", False) == True:
            ...
            self.completions: Union[
                AsyncCompletions, Completions
            ] = AsyncCompletions(self.params, router_obj=router_obj)
        else:
    ...
    **kwargs,
    ...
    try:
        ...
        if isinstance(mock_response, Exception):
            ...
            and mock_response == "litellm.RateLimitError"
        ):
    ...
    try:
        ...
        if (
            ...
            if isinstance(init_response, dict):
                ...
                init_response, EmbeddingResponse
            ):  ## CACHING SCENARIO
                response = init_response
            elif asyncio.iscoroutine(init_response):
                response = await init_response
    ...
    **kwargs,
    ...
    try:
        ...
        if azure is True or custom_llm_provider == "azure":
            ...
            gemini_api_key = (
                api_key or get_secret("GEMINI_API_KEY") or litellm.api_key
            )
            ...
            api_key = (
                api_key or litellm.api_key or get_secret("MISTRAL_API_KEY")
            )
            response = openai_chat_completions.embedding(
                model=model,
                input=input,
            ...
            api_key = (
                api_key or litellm.api_key or get_secret("VOYAGE_API_KEY")
            )
            response = openai_chat_completions.embedding(
                model=model,
                input=input,
            ...
            args = locals()
            raise ValueError(
                f"No valid embedding model args passed in - {args}"
            )
        if response is not None and hasattr(response, "_hidden_params"):
            response._hidden_params[
                "custom_llm_provider"
    ...
    *args, **kwargs
    ...
    try:
        ...
        if kwargs.get("stream", False) is True:  # return an async generator
            ...
            try:
                raw_response = response._hidden_params.get(
                    "original_response", None
                )
                transformed_logprobs = litellm.utils.transform_logprobs(
                    raw_response
                )
            except Exception as e:
            ...
            text_choices["finish_reason"] = response["choices"][0][
                "finish_reason"
            ]
...
def text_completion(
    ...
    model: Optional[
        str
    ] = None,  # Optional: either `model` or `engine` can be set
    best_of: Optional[
        int
    ...
    stream: Optional[
        bool
    ] = None,  # Optional: Whether to stream back partial progress.
    stream_options: Optional[dict] = None,
    suffix: Optional[
        str
    ...
    temperature: Optional[
        float
    ] = None,  # Optional: Sampling temperature to use.
        ...
        if type(prompt) == list:
            ...
            if len(prompt) > 0 and type(prompt[0]) == list:
                ...
                def process_prompt(i, individual_prompt):
                    ...
                    text_completion_response["id"] = response.get("id", None)
                    text_completion_response["object"] = "text_completion"
                    text_completion_response["created"] = response.get(
                        "created", None
                    )
                    text_completion_response["model"] = response.get(
                        "model", None
                    )
...
async def aadapter_completion(
    *, adapter_id: str, **kwargs
    ...
    try:
        ...
        new_kwargs = translation_obj.translate_completion_input_params(
            kwargs=kwargs
        )
        ...
        if isinstance(response, ModelResponse):
            translated_response = (
                translation_obj.translate_completion_output_params(
                    response=response
                )
            )
        if isinstance(response, CustomStreamWrapper):
...
def adapter_completion(
    *, adapter_id: str, **kwargs
    ...
    elif isinstance(response, CustomStreamWrapper) or inspect.isgenerator(
        response
    ):
        translated_response = (
            translation_obj.translate_completion_output_params_streaming(
...
async def ahealth_check(
    ...
    try:
        ...
        if custom_llm_provider == "azure":
            ...
            api_key = model_params.get("api_key") or get_secret(
                "OPENAI_API_KEY"
            )
            ...
            api_base = model_params.get("api_base") or get_secret(
                "OPENAI_API_BASE"
            )
...
def stream_chunk_builder(
    ...
    try:
        ...
        if chunks[0]._hidden_params.get("created_at", None):
            ...
            chunks = sorted(
                chunks,
                key=lambda x: x._hidden_params.get("created_at", float("inf")),
            )
        ...
        for chunk in chunks:
            ...
            if "usage" in chunk:
                ...
                and "usage" in chunk._hidden_params
            ):
            ...
            if usage_chunk is not None:
                ...
                if "completion_tokens" in usage_chunk:
                    completion_tokens = (
                        usage_chunk.get("completion_tokens", 0) or 0
                    )
                ...
                if "cache_read_input_tokens" in usage_chunk:
                    cache_read_input_tokens = usage_chunk.get(
        ...
        try:
            response["usage"][
                "prompt_tokens"
            ] = prompt_tokens or token_counter(model=model, messages=messages)
        except (
            Exception
        ...
        response["usage"][
            "completion_tokens"
        ] = completion_tokens or token_counter(
            model=model,
